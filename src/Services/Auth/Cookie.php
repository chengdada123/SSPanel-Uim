...
final class Cookie extends Base
{
    public function login($uid, $time): void
    {
        ...
        CookieUtils::set([
            ...
            'ip' => Hash::ipHash($_SERVER['HTTP_X_FORWARDED_FOR'], $uid, $expire_in),
    ...
    public function getUser(): User
    {
        ...
        if ($_ENV['enable_login_bind_ip']) {
            $nodes = Node::where('node_ip', $_SERVER['HTTP_X_FORWARDED_FOR'])->first();

            if (($nodes === null) && $ipHash !== Hash::ipHash($_SERVER['HTTP_X_FORWARDED_FOR'], $uid, $expire_in)) {
                return $user;
            }
        }
